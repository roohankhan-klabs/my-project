        ...
        <section class="rounded-xl bg-white p-5 shadow-sm border border-slate-100 space-y-6">
          ...
          <div>
            ...
            <div v-if="filesStore.currentFolder" class="mb-4">
              <button
                @click="navigateTo(filesStore.currentFolder.parent_id)"
                @dragover.prevent
            ...
            <ul v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              <FolderCard
                ...
                @drop-file="handleMoveFile"
                @drop-folder="handleMoveFolder" />
            </ul>
          </div>
          ...
          <div>
            ...
            <ul v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              <FileCard
                ...
                @download="filesStore.downloadFile($event)" />
            </ul>
          </div>
...
async function handleMoveFile(fileId, targetFolderId) {
  try {
    await filesStore.moveFile(Number(fileId), Number(targetFolderId))
...
async function handleMoveFolder(folderId, parentFolderId) {
  try {
    await filesStore.moveFolder(Number(folderId), Number(parentFolderId))
